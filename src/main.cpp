...
int main() {
  ...
  // Lane Variable starting in Lane 1
  int lane = 1;
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // Assign, if the previous path has some values
          if (prev_size > 0) {
            car_s = end_path_s;
          }
          
          // Analysing other car positions
          ...
          // Decide the next state based on analysis
          double speed_change = 0;
          // If a car is in front
          if (car_ahead) {
            if (!car_left && lane > 0) {
              // No cars on left and left lane change possible
              lane -= 1;
            }
            else if (!car_right && lane < 2) {
              // No cars on right and right lane change possible
              lane += 1;
            }
            else {
          ...
          // If there is no car in front
            ...
            if (lane != 1) {
              // If not on center lane and can be changed to center lane
          ...
          // Then interpolate these waypoints with a spline
          vector<double> ptsx;
          vector<double> ptsy;
          
          // Reference x, y and yaw states
          // Either use starting point or previous end point as to where the car is
          double ref_x = car_x;
          double ref_y = car_y;
          ...
          // Calculate the break up spline points to travel at reference velocity
          ...
          for (int i = 1; i < 50 - previous_path_x.size(); i += 1){
            ...
            // Rotate back to normal after rotating it
